...
def transfer_select(self):
    ...
    for root, dirs, files in os.walk(path):
        for filename in files:
            ...
            # Get date of last file transfer from the database:
            conn = sqlite3.connect('move.db')
            with conn:
                cur = conn.cursor()
                cur.execute(""" SELECT datestamp FROM move_table""")
                today_date = cur.fetchone()[0]

                # Get dates for new and modded files
                new_file = str(datetime.date.fromtimestamp(os.stat(filename).st_ctime))
                mod_file = str(datetime.date.fromtimestamp(os.stat(filename).st_mtime))
                ...
                if mod_file <= today_date or new_file <= today_date:
                    ...
                    with conn:
                        ...
                        unix = int(time.time())
                        date = str(datetime.datetime.fromtimestamp(unix).strftime('%Y-%m-%d %H:%M:%S'))
                        c.execute("""INSERT INTO move_table(unix,datestamp) VALUES (?,?)""",(unix, date))
                        conn.commit()
                else:
