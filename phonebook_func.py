...
def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    ...
    # calculate x and u coordinates to pain the app centered on the users screen
...
def onSelect(self, event):
    # Calling the event is the self.1stList widget
...
def addToList(self):
    ...
    var_lname = var_lname.strip() # This will ensure that the character in each word is capitalized
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    crusor.execute(""" DELETE FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_select))
...
def onUpdate(self):
    try:
        ...
        return
    # The use will only be alowed to update changes for the phone and emials
    ...
    var_phone = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # Ensure that there is data
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cur = conn.cursor()
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request", " The following changes({}) and ({}) will be implemented for ({})".format(var_phone,var_email,var_fullname))
                print(response)
                if response:
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute(""" UPDATE tbl_phonebook SET col_phone = '{}',col_email'{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_fullname))
                        onClear(self)
                        conn.commit()
                else:
